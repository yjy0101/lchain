...
# file_path 表示配置文件路径，默认值为 "mcp.json"
# 返回字典（Dict），键是字符串（服务器标识），值是任意类型（服务器配置）
def load_servers(file_path: str = "mcp.json") -> Dict[str, Any]:
    ...
    with open(file_path, "r", encoding="utf-8") as file:
        data = json.load(file) # 功能：使用Python的 json 模块解析文件内容；作用：将JSON格式的文本转换为Python字典/列表等数据结构
...
async def run_chat_loop() -> None:
    ...
    """
    # 1️⃣ 加载服务器配置
    servers_cfg = load_servers() # 默认值为"mcp.json"
